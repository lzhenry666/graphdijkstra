...
(function () {
     ...
     * ] }
     */
    var Graph = function Graph(graph) {
        var i = 0;
        ...
        if (!('nodes' in graph)) {
            throw new Error('Invalid graph format: must specify array \'nodes\' with keys' + ' \'id\' and \'props\' (\'props\' has keys \'weight\', \'nType\', \'neighbors\')');
        }
        if (!('edges' in graph)) {
            throw new Error('Invalid graph format: must specify array \'edges\' with elements' + ' of the form [ sourceID, targetID ]');
        }
